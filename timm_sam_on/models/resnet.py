...
from timm_sam_on.data import IMAGENET_DEFAULT_MEAN, IMAGENET_DEFAULT_STD
...
def downsample_conv(
    ...
    d = nn.Sequential()
    d.add_module('conv2d', nn.Conv2d(
            in_channels, out_channels, kernel_size, stride=stride, padding=p, dilation=first_dilation, bias=False))
    d.add_module('norm', norm_layer(out_channels))
    return d
...
def downsample_avg(
    ...
    d = nn.Sequential()
    d.add_module('pool', pool)
    d.add_module('conv2d', nn.Conv2d(in_channels, out_channels, 1, stride=1, padding=0, bias=False))
    d.add_module('norm', norm_layer(out_channels))
    return d
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, in_chans=3,
        ...
        if deep_stem:
            ...
            self.conv1.add_module('conv2d1', nn.Conv2d(in_chans, stem_chs[0], 3, stride=2, padding=1, bias=False))
            self.conv1.add_module('norm1', norm_layer(stem_chs[0]))
            self.conv1.add_module('act1', act_layer(inplace=True))
            self.conv1.add_module('conv2d2', nn.Conv2d(stem_chs[0], stem_chs[1], 3, stride=1, padding=1, bias=False))
            self.conv1.add_module('norm2', norm_layer(stem_chs[1]))
            self.conv1.add_module('act2', act_layer(inplace=True))
            self.conv1.add_module('conv2d3', nn.Conv2d(stem_chs[1], inplanes, 3, stride=1, padding=1, bias=False))
        else:
        ...
        if replace_stem_pool:
            ...
            self.maxpool.add_module('conv2d', nn.Conv2d(inplanes, inplanes, 3, stride=1 if aa_layer else 2, padding=1, bias=False))
            if aa_layer:
                self.maxpool.add_module('aa_layer', aa_layer(channels=inplanes, stride=2))
            self.maxpool.add_module('norm', norm_layer(inplanes))
            self.maxpool.add_module('act', act_layer(inplace=True))
        else:
