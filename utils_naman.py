...
Custom implementation: ConvStem (ConvBlock) with standard models built on timm_esam.
'''
...
from timm_sam_on.models import create_model
#from timm_esam.models.convnext import _create_convnext as CNXT
import torch.nn.functional as F
from functools import partial
import math
#from timm_esam.models.vision_transformer import VisionTransformer
...
def get_new_model(modelname, pretrained=True, not_original=False, updated=False):
    ...
    if modelname == 'resnet50':
        model = timm_sam_on.models.resnet.resnet50(pretrained=pretrained)
...
#             setattr(model, 'stem', ConvBlock(48, end_siz=8, fin_dim=432 if updated else 384))
        ...
        model = timm_sam_on.models.convnext.convnext_tiny(pretrained=pretrained)
        if not_original:
            model.stem = ConvBlock1(48, end_siz=8)

    elif modelname == "convnext_tiny_21k":
        model = timm_sam_on.models.convnext._create_convnext('convnext_tiny.fb_in22k_ft_in1k', pretrained=pretrained)

    elif modelname == "convnext_small":
        model_args = dict(depths=[3, 3, 27, 3], dims=[96, 192, 384, 768])
        model = timm_sam_on.models.convnext.convnext_small(pretrained=pretrained)
        if not_original:
        ...
        # model = timm_esam.models.create_model('convnext_base', pretrained=pretrained, pretrained_cfg='convnext_base.fb_in1k')
        model = timm_sam_on.models.convnext._create_convnext('convnext_base.fb_in1k', pretrained=pretrained, **model_args)
        if not_original:
            ##   only for removing patch-stem 
        ...
        model = timm_sam_on.models.convnext_large(pretrained=pretrained)
        ...
        from timm_sam_on.models.deit import deit3_small_patch16_224, _create_deit
        model_kwargs = dict(
        ...
        model = timm_sam_on.models.deit.deit3_medium_patch16_224(pretrained=pretrained)
        if not_original:
            ##   only for removing patch-stem 
        ...
        model = timm_sam_on.models.vision_transformer.vit_base_patch16_224(pretrained=pretrained) # used for 21k pretrained on 206
        ...
        model = timm_sam_on.models.resnet.resnet101(pretrained=False)

    elif modelname=="wrn_50_2":
        model = timm_sam_on.models.resnet.wide_resnet50_2(pretrained=False)

    elif modelname == "densnet201":
        model = timm_sam_on.models.densenet.densenet201(pretrained=pretrained)
